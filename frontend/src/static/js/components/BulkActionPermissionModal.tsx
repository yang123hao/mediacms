...
export const BulkActionPermissionModal: React.FC<BulkActionPermissionModalProps> = ({
  ...
  const fetchExistingUsers = async () => {
    ...
    try {
      ...
      onError('Failed to load existing permissions');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      const permissionLabel = permissionType === 'viewer' ? 'Co-Viewers' : permissionType === 'editor' ? 'Co-Editors' : 'Co-Owners';
      onSuccess(`Successfully updated ${permissionLabel}`);
      onCancel();
    } catch (error) {
      console.error('Error processing permissions:', error);
      onError('Failed to update permissions. Please try again.');
    } finally {
        ...
        <div className="permission-modal-content">
          <div className="permission-panel">
            ...
            <div className="user-list">
              {usersToAdd.length === 0 ? (
                <div className="empty-message">No users to add</div>
              ) : (
                usersToAdd.map((user) => (
          ...
          <div className="permission-panel">
            ...
            <div className="search-box">
              <input
                type="text"
                placeholder="Filter existing users..."
            ...
            {isLoading ? (
              <div className="loading-message">Loading existing users...</div>
            ) : (
              <div className="user-list">
                {filteredExistingUsers.length === 0 ? (
                  <div className="empty-message">No existing {permissionLabel.toLowerCase()}</div>
                ) : (
                    ...
                    return (
                      <div key={user} className={`user-item ${isMarkedForRemoval ? 'marked-for-removal' : ''}`}>
                        ...
                        <button
                          ...
                          title={isMarkedForRemoval ? 'Undo removal' : 'Remove user'}
                        >
                          {isMarkedForRemoval ? '↺' : '×'}
                        </button>
                      </div>
